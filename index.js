...
const SOURCE_URL = process.env.SOURCE_URL || 'http://201.251.63.225:88/meteo/';
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .btn {
      background: #1d4ed8; color: #fff; border: none; border-radius: 0.5rem;
...
<body>
  <div class="card">
    ...
    <table id="data-table">${tableRows}
    </table>
    <div class="footer">
      <button class="btn" id="refresh-btn" onclick="refreshData()">Actualizar</button>
      <p class="updated" id="updated-time">Actualizado: ${updatedAt}</p>
    </div>
  </div>
  <script>
    async function refreshData() {
      ...
      try {
        ...
        const kmhToKnots = v => v ? (parseFloat(v.replace(',','.').replace(/[^0-9.\-]/g,'')) * 0.539957).toFixed(1) : null;
        const rows = [
          ['Temperatura',       d.temperature  ? d.temperature + ' °C'  : '—'],
          ['Sensación térmica', d.feelsLike    ? d.feelsLike + ' °C'    : '—'],
          ['Viento',            kmhToKnots(d.windSpeed) ? kmhToKnots(d.windSpeed) + ' kn' : '—'],
        ...
        document.getElementById('data-table').innerHTML = rows.map(([label, value]) =>
          '<tr><td class="label">' + label + '</td><td class="value">' + value + '</td></tr>'
        ).join('');
        const now = new Date().toLocaleString('es-AR', { timeZone: 'America/Argentina/Ushuaia', hour12: false });
        document.getElementById('updated-time').textContent = 'Actualizado: ' + now;
      } catch(e) {
