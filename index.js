...
// NUEVA FUNCIÓN: Actualiza la cache sin hacer esperar al cliente
async function refreshCache() {
  try {
    ...
    console.log("✅ Datos actualizados en cache: " + data.stationTime);
    return data;
  } catch (e) {
    console.error("❌ Error refrescando cache:", e.message);
    return null;
  }
}
...
// 1. Endpoint para Garmin (RESPUESTA INSTANTÁNEA)
app.get('/weather-view', async (req, res) => {
  let data = weatherCache.get("weather_data");

  if (!data) {
    // Si no hay nada, tenemos que esperar sí o sí la primera vez
    data = await refreshCache();
    if (!data) return res.status(502).json({ error: "Estación offline" });
  } else {
    // SI HAY DATOS: Los enviamos ya, y refrescamos la cache por detrás
    // para que la próxima consulta esté actualizada.
    refreshCache(); 
  }
...
app.get('/', async (req, res) => {
  let data = weatherCache.get("weather_data");
  if (!data) data = await refreshCache();
  
  if (!data) return res.status(502).send("Error al obtener datos");
...
// --- TAREA AUTOMÁTICA WINDGURU (Cada 2 min) ---
setInterval(async () => {
  if (!WG_UID || !WG_PASSWORD) return;
  
  // Forzamos actualización de datos antes de subir a Windguru
  const d = await refreshCache();
  if (!d) return;
  ...
  try {
    ...
    console.log("☁️ Windguru actualizado");
